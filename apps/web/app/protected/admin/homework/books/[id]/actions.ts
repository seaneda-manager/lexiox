...
export type BookLevelInfo = {
  ...
  expectedSchoolGrade: string;
  expectedMockExam: string;
};

export async function updateBookLevelInfoAction(
  bookId: string,
  info: BookLevelInfo,
): Promise<{ ok: true } | { error: string }> {
  const supabase = await getServerSupabase();
  ...
  const { error } = await service
    .from('homework_books')
    .update({
      level: info.level.trim() || null,
      recommended_grade: info.recommendedGrade.trim() || null,
      expected_school_grade: info.expectedSchoolGrade.trim() || null,
      expected_mock_exam: info.expectedMockExam.trim() || null,
    })
    .eq('id', bookId);
  if (error) return { error: error.message };

  revalidatePath(`/admin/homework/books/${bookId}`);
  revalidatePath('/admin/homework/books');
  return { ok: true };
}
