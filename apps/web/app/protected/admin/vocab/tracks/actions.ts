 ...
 * 선택된 세트들을 트랙과 연결
 * ======================================================= */
export async function linkSetsToTrackAction(
  setIds: string[],
  trackId: string
): Promise<{ ok: boolean; error?: string }> {
  try {
    if (!setIds.length || !trackId) {
      return { ok: false, error: "setIds and trackId are required" };
    }
    ...
    return { ok: false, error: e?.message ?? "Failed to link sets to track" };
  }
}
...
/* =========================================================
 * Admin: Update assignment day (move between days)
 * ======================================================= */
export async function updateAssignmentDayAction(params: {
  ...
  try {
    ...
    const assignmentId = cleanStr(params.assignmentId);
    const newDayIndex = clampInt(params.newDayIndex, 1, 9999, 1);
    ...
    const { data: row, error: rerr } = await supabase
      .from("student_vocab_assignments")
      .select("id, student_id, track_id, day_index")
      .eq("id", assignmentId)
      .maybeSingle();
    ...
    const { error: uerr } = await supabase
      .from("student_vocab_assignments")
      .update({ day_index: newDayIndex } as any)
      .eq("id", assignmentId);

    if (uerr) throw new Error("update failed");

    return { ok: true, assignmentId, newDayIndex };
  } catch (e: any) {
    return { ok: false, error: e?.message ?? "failed" };
  }
}
...
/* =========================================================
 * Admin: Reorder assignments (drag & drop support)
 * ======================================================= */
export async function reorderAssignmentsAction(params: {
  assignments: Array<{ id: string; dayIndex: number }>;
}) {
  try {
    const supabase = await getServerSupabase();
    await getUserOrThrow(supabase);
    ...
    for (const asg of assignments) {
      const assignmentId = cleanStr(asg.id);
      const newDayIndex = clampInt(asg.dayIndex, 1, 9999, 1);
