...
export default function TrackAssignClient({
  ...
  // 큐 편집 상태
  const [editingQueue, setEditingQueue] = useState<Record<string, number>>({}); // assignmentId -> newDayIndex
  ...
  async function updateAssignmentDay(assignmentId: string) {
    const newDay = editingQueue[assignmentId];
    if (!newDay || newDay < 1) {
      setMsg("❌ 유효한 Day를 입력하세요");
      return;
    }
    ...
    setBusy(true);
    try {
      const res = await updateAssignmentDayAction({ assignmentId, newDayIndex: newDay });
      if (!res.ok) {
        setMsg(`❌ ${res.error}`);
        return;
      }
      setMsg("✅ Day 변경 완료");
      setEditingQueue(prev => { const next = { ...prev }; delete next[assignmentId]; return next; });
      await loadPlan();
    } catch (e: any) {
      setMsg(`❌ ${e?.message ?? "failed"}`);
    } finally {
                ...
                {queue.map((r) => (
                  <tr key={r.id} className="border-b last:border-0 hover:bg-slate-50">
                    <td className="py-2 pr-4 font-bold">
                      {editingQueue[r.id] !== undefined ? (
                        <input
                          type="number"
                          min="1"
                          value={editingQueue[r.id]}
                          onChange={(e) => setEditingQueue(prev => ({ ...prev, [r.id]: Number(e.target.value) }))}
                      ...
                      {editingQueue[r.id] !== undefined ? (
                        <>
                          <button
                            type="button"
                            disabled={busy}
                            onClick={() => updateAssignmentDay(r.id)}
                            className="rounded-lg bg-blue-600 hover:bg-blue-700 px-3 py-1 text-xs font-bold text-white disabled:opacity-30"
                          >
                            저장
                          </button>
                          <button
                            type="button"
                            disabled={busy}
                            onClick={() => setEditingQueue(prev => { const next = { ...prev }; delete next[r.id]; return next; })}
                            className="rounded-lg border px-3 py-1 text-xs font-bold disabled:opacity-30 hover:bg-slate-100"
                          >
                            취소
                          </button>
                        </>
                        ...
                        <>
                          <button
                            type="button"
                            disabled={busy}
                            onClick={() => setEditingQueue(prev => ({ ...prev, [r.id]: r.day_index }))}
                            className="rounded-lg border px-3 py-1 text-xs font-bold disabled:opacity-30 hover:bg-blue-50 hover:border-blue-300 hover:text-blue-700"
                          >
                            변경
                          </button>
                          <button
