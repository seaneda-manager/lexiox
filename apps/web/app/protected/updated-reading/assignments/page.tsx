...
export default async function ReadingAssignmentsPage() {
  ...
  const { data: assignments } = await supabase
    ...
    .select(`
      ...
      reading_test_id,
      reading_tests!test_assignments_reading_test_id_fkey (
        id, label
      )
    `)
    .eq("student_id", user.id)
    .contains("sections", ["reading"])
    ...
    <main className="mx-auto max-w-2xl space-y-5 px-4 py-6">
      ...
      {list.length === 0 ? (
        ...
        <div className="space-y-3">
          {list.map((a) => {
            const test = Array.isArray(a.reading_tests) ? a.reading_tests[0] : a.reading_tests;
