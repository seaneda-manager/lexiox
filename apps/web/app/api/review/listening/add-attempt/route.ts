...
import { createReviewAttempt, type ReviewAttempt } from "@/lib/review/reviewTracker";

export async function POST(req: Request) {
  try {
    const body = await req.json();
    const { resultId, questionId, stage } = body;
    ...
    const updatedAttempts = createReviewAttempt(currentAttempts, stage);
    ...
    if (updateErr) {
      console.error("[listening/add-attempt] update error:", updateErr);
      return NextResponse.json(
    ...
    console.error("[listening/add-attempt] error:", error);
    return NextResponse.json(
