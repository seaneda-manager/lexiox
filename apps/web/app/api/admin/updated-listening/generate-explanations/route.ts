...
export async function POST(req: Request) {
  try {
    const { transcript, question, choices } = await req.json();

    if (!transcript || !question || !choices || choices.length === 0) {
      return Response.json(
        { error: "transcript, question, choices are required" },
        { status: 400 }
      );
    }

    const prompt = `음성 스크립트(transcript)와 문제(question), 선택지(choices)가 주어졌습니다.
각 선택지를 문법 구조에 따라 직독직해하고, 의미를 분석해주세요.
