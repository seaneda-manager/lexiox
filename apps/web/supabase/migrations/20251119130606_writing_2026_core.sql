...
drop policy if exists "writing_2026_sessions_select_own" on public.writing_2026_sessions;
create policy "writing_2026_sessions_select_own"
  on public.writing_2026_sessions
  for select
  using (auth.uid() = user_id);

drop policy if exists "writing_2026_sessions_insert_own" on public.writing_2026_sessions;
